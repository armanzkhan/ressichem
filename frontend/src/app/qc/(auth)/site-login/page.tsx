...
function QCSiteLoginContent() {
  ...
  useEffect(() => {
    ...
    if (hasQcPortalSession() || hasValidQcSession()) {
      ...
      const safeNext = next && next.startsWith("/qc/site") ? next : "/qc/site/results";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch("/api/qc/auth/site-login", {
        method: "POST",
      ...
      if (!response.ok) {
        setMessage(data?.message || "QC Site login failed");
        return;
      }
      if (!data?.token) {
      ...
      setMessage("QC Site login failed. Please try again.");
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="rounded-xl sm:rounded-2xl bg-white/80 backdrop-blur-sm shadow-2xl border border-white/20 dark:bg-gray-800/80 dark:border-gray-700/20 p-6 sm:p-8">
          ...
          <div className="text-center mb-8">
            ...
            <div className="flex items-center justify-center gap-2 text-sm text-blue-900 dark:text-blue-300 flex-wrap">
              <span className="px-3 py-1 bg-blue-100 dark:bg-blue-900/30 rounded-full font-medium">Tests</span>
              <span className="w-1 h-1 bg-gray-400 rounded-full"></span>
              <span className="px-3 py-1 bg-blue-100 dark:bg-blue-900/30 rounded-full font-medium">Standards</span>
              <span className="w-1 h-1 bg-gray-400 rounded-full"></span>
              <span className="px-3 py-1 bg-blue-100 dark:bg-blue-900/30 rounded-full font-medium">Results</span>
            </div>
          </div>
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
            ...
            <div className="text-center space-y-3 text-xs text-blue-900 dark:text-blue-300 pt-2">
              <div>
                Need QC Hub?{" "}
                <Link href="/qc/hub-login" className="font-semibold text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 hover:underline transition-colors">
                  Go to QC Hub Login
                </Link>
              </div>
              <div>
                New user?{" "}
                <Link href="/qc/site-signup" className="font-semibold text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 hover:underline transition-colors">
                  Create QC Site Account
                </Link>
              </div>
...
export default function QCSiteLoginPage() {
  return (
