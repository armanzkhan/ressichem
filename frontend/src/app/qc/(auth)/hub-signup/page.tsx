...
export default function QCHubSignupPage() {
  const [formData, setFormData] = useState({
    ...
    password: "",
    confirmPassword: "",
    role: "QC Hub User", // Default permission-based role
    layer: "", // SRS 2.3 Three-Layer Model
    layerRole: "", // Specific role within the layer
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.layer || !formData.layerRole) {
    ...
    try {
      const payload = {
        ...
        layerRole: formData.layerRole,
      };

      const response = await fetch("/api/qc/auth/hub-signup", {
        method: "POST",
      ...
      if (!response.ok) {
        setMessage({ type: "error", text: data?.message || "QC Hub signup failed" });
        return;
      }

      setMessage({ type: "success", text: "Account created. You can now login to QC Hub." });
      setFormData((prev) => ({
        ...prev,
        firstName: "",
        lastName: "",
      ...
      setMessage({ type: "error", text: "QC Hub signup failed. Please try again." });
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        <div className="rounded-xl sm:rounded-2xl bg-white/80 backdrop-blur-sm shadow-2xl border border-white/20 dark:bg-gray-800/80 dark:border-gray-700/20 p-6 sm:p-8 relative overflow-hidden">
          ...
          <div className="text-center mb-6 relative z-10">
            ...
            <p className="text-sm text-blue-900 dark:text-blue-300">Create your account to access Quality Control Hub</p>
          </div>
          ...
          <form className="space-y-3 relative z-10" onSubmit={handleSubmit}>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  type="text"
                  readOnly
                  className="w-full rounded-lg border-2 border-blue-200 dark:border-blue-800 bg-blue-50/50 dark:bg-blue-900/20 py-2 pl-10 pr-20 text-sm font-semibold text-blue-700 dark:text-blue-300 cursor-not-allowed"
                  value={formData.company_id}
                />
                <div className="absolute inset-y-0 right-0 pr-3 flex items-center">
                  <span className="inline-flex items-center px-2 py-0.5 rounded-md bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-300 text-xs font-semibold">
                    Read Only
                  </span>
                </div>
              </div>
            ...
            {/* Permission-Based Role */}
            <div>
              <label className="block text-xs font-semibold text-blue-900 dark:text-blue-300 mb-1.5 uppercase tracking-wide">
                Select Your Role * <span className="text-blue-600 dark:text-blue-400">(SRS 2.3)</span>
              </label>
              <div className="relative">
                ...
                >
                  <option value="QC Hub User">🏢 QC Hub User - Basic QC Hub access</option>
                  <option value="QC Analyst">🔬 QC Analyst - Access both portals</option>
                  <option value="QC Manager">👔 QC Manager - Full access with approvals</option>
                  <option value="QC Admin">⚙️ QC Admin - Full access + user management</option>
                  <option value="QC Viewer">👁️ QC Viewer - Read-only access</option>
                </select>
            ...
            <div>
              <label className="block text-xs font-semibold text-blue-900 dark:text-blue-300 mb-1.5 uppercase tracking-wide">
                Select Your Layer * <span className="text-blue-600 dark:text-blue-400">(SRS 2.3)</span>
              </label>
              <div className="relative">
                <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none z-10">
                  <svg className="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  >
                    ...
                    {formData.layer === "QC" && (
                      <>
                        <option value="QC_LAB_TECHNICIAN">🔬 QC Lab Technician - Execute QC testing</option>
                        <option value="QC_SUPERVISOR">👨‍💼 QC Supervisor - Supervise QC operations</option>
                      </>
                    )}
                    {formData.layer === "R&D" && (
                      <>
                        <option value="R&D_CHEMIST">⚗️ R&D Chemist - Develop formulations</option>
                        <option value="SENIOR_R&D_SCIENTIST">🧪 Senior R&D Scientist - Lead R&D projects</option>
                      </>
                    )}
                    {formData.layer === "MANAGEMENT" && (
                      <>
                        <option value="TECHNICAL_MANAGER_RND">🔧 Technical Manager (R&D) - Approve R&D→QC</option>
                        <option value="PLANT_HEAD">🏭 Plant Head - Plant-level oversight</option>
