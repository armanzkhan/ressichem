...
export default function CreateQCSiteUserPage() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setMessage("");
    try {
      const res = await fetch(`${apiUrl}/api/qc/users/create-site`, {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
        body: JSON.stringify(form),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data?.message || "Failed to create QC Site user");
      setMessage("✅ QC Site user created");
  ...
  return (
    <div className="max-w-2xl space-y-6">
      <div className="rounded-2xl bg-white/80 dark:bg-gray-800/80 border border-white/30 dark:border-gray-700/50 shadow p-6">
        <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Create QC Site User</h2>
        <p className="text-sm text-gray-600 dark:text-gray-400 mt-2">
          This creates a user with role <span className="font-semibold">QC Site User</span> and module tag <span className="font-mono">QC_SITE</span>.
        </p>
      </div>
        ...
        <form onSubmit={submit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="md:col-span-2 flex items-center justify-between gap-3">
            ...
            <button
              ...
              type="submit"
            >
              {loading ? "Creating…" : "Create QC Site User"}
            </button>
          </div>
        </form>
      </div>
