...
async function testFrontendBackendConnection() {
  ...
  try {
    ...
    const response = await fetch(`${FRONTEND_URL}/api/test-connection`, {
      method: 'GET',
      headers: { 'Content-Type': 'application/json' },
      timeout: 10000
    });
...
async function testQCSignupEndpoint() {
  ...
  try {
    ...
    const response = await fetch(`${BACKEND_URL}/api/qc/auth/site-signup`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    const success = response.status === 200 || response.status === 400 || response.status === 409;
